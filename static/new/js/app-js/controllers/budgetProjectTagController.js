angular.module('swire').controller('budgetProjectTagController',
	function($scope, $rootScope, $filter, $state,
		...
		$scope.getFilteredBudgets = function() {
			...
			console.log('here');
			budgetServices.getBudgetProject({customerName: $scope.customerName,budgetName:$scope.budgetName}).$promise.then(function(response) {
				$scope.selectedRowId = 0;
	...
	$scope.tag = function (){
		...
		budgetServices.tagBudgetProject({customerName: $scope.customerName,projectName: $scope.filteredBuBudgets[$scope.selectedRowId].buname,budgetName:$scope.budgetName}).$promise.then(function(response) {
			$scope.getFilteredUnTaggedBudgets();
		})
	}
	$scope.untag = function (){
		budgetServices.unTagBudgetProject({customerName: $scope.customerName,projectName: $scope.filteredBuBudgets[$scope.selectedRowId].buname,budgetName:$scope.budgetName}).$promise.then(function(response) {
	...
	$scope.selectRowData = function (rowKey) {
		...
		$scope.getFilteredUnTaggedBudgets = function() {
			var selectedServiceName = $scope.serviceName;
			budgetServices.getUntaggedBudgetProject({customerName: $scope.customerName,budgetName:$scope.budgetName}).$promise.then(function(response) {
				$scope.selectedRowId = 0;
				$scope.filteredBuBudgets = response['statusMessage'].map(function (bu) {
					return {
						'buname': bu,
